...
const Missions = () => {
    ...
    useEffect(() => {
        const fetchMissions = async () => {
            try {
                setLoading(true);
                const data = await getCollection('missions');
                setMissions(data);
                setError(null);
            } catch (err) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this mission?')) {
            try {
                await deleteDocument('missions', id);
                        ...
                        <div key={mission.id} className="bg-gray-800 rounded-lg shadow-lg overflow-hidden">
                            <div className="p-6">
                                ...
                                <div className="text-sm text-gray-500 mb-4">
                                    <p>Start Date: {mission.startDate ? new Date(mission.startDate).toLocaleDateString() : 'N/A'}</p>
                                    <p>End Date: {mission.endDate ? new Date(mission.endDate).toLocaleDateString() : 'N/A'}</p>
                                </div>
