...
const GroundStations = () => {
    ...
    const handleAddGroundStation = async (e) => {
        ...
        try {
            ...
            const formattedData = {
                name: newGroundStation.name.trim(),
                location: newGroundStation.location.trim(),
                latitude: parseFloat(newGroundStation.latitude),
                longitude: parseFloat(newGroundStation.longitude),
                elevation: newGroundStation.elevation ? parseFloat(newGroundStation.elevation) : null,
                status: newGroundStation.status,
                description: newGroundStation.description.trim() || null
            };

            // Validate required fields
            if (!formattedData.name || !formattedData.location || 
                isNaN(formattedData.latitude) || isNaN(formattedData.longitude)) {
                throw new Error('Please fill in all required fields with valid values');
            }

            // Create new ground station using API
            const newStationData = await API.groundStations.create(formattedData);
            ...
            alert(error.message || "Failed to add ground station. Please try again.");
        }
    };
        ...
        <div className="container mx-auto px-4 py-8">
            <div className="flex justify-between items-center mb-6">
                ...
                <button
                    ...
                    className="px-4 py-2 bg-space-blue text-white rounded-md hover:bg-space-blue-dark transition"
                >
